<template>
  <b-container class="mt-5">
    <b-row>
      <b-col cols="12">
        <b-card :title="realm ? `Banque de ${realm.name}`: ''"
...
<script>
  export default {
    ...
    methods: {
      getRealm () {
        window.axios
          .get('/api/realms/' + this.$route.params.id)
          .then(response => {
      ...
      transactionsProvider (ctx) {
        return window.axios
          .get(`api/realms/${this.$route.params.id}/transactions`, {params: {page: ctx.currentPage}})
          .then(response => {
